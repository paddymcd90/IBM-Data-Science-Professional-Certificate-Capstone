...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        success_counts = spacex_df.groupby('Launch Site', as_index=False)['class'].sum()
...
def update_scatter_chart(site_dropdown, payload_slilder):
    filtered_df = spacex_df[
        (spacex_df['Payload Mass (kg)'] >= payload_slilder[0]) & 
        (spacex_df['Payload Mass (kg)'] <= payload_slilder[1])
    ]
    ...
    if site_dropdown == "ALL":
        ...
        site_filtered_df = filtered_df[filtered_df['Launch Site'] == site_dropdown]
        fig = px.scatter(
            site_filtered_df, 
            x='Payload Mass (kg)', 
            y='class', 
